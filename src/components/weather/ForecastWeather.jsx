...
import { formatTemperature, formatDate, formatPrecipitation } from '../../lib/utils';
import { Card, CardHeader, CardTitle, CardContent } from '../ui/Card';
import ForecastChart from '../charts/ForecastChart';
import HourlyChart from '../charts/HourlyChart';

export default function ForecastWeather() {
  const { location: locationName, unit } = useSelector((state) => state.weather);
  const [selectedDayIndex, setSelectedDayIndex] = useState(0);

  const { data: forecastData, isLoading, error } = useGetForecastQuery({
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center min-h-[50vh]">
        <div className="animate-pulse text-xl text-gray-500">Loading forecast data...</div>
      </div>
    );
  }
    ...
    return (
      <div className="flex justify-center items-center min-h-[50vh]">
        <div className="text-red-500 text-center">
          ...
          <p>{error?.data?.error?.message || 'Unable to fetch forecast information'}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6">
            ...
            >
              <div className="flex flex-col items-center">
                ...
                <div className="flex gap-2 text-sm">
                  <span className="font-medium">{formatTemperature(day.day.maxtemp_c, unit)}</span>
                  <span className="text-gray-500">{formatTemperature(day.day.mintemp_c, unit)}</span>
                </div>
              </div>
            </motion.button>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Hourly Temperature ({formatDate(selectedDay.date)})</CardTitle>
            </CardHeader>
            <CardContent>
              <HourlyChart data={forecast} dayIndex={selectedDayIndex} unit={unit} />
            </CardContent>
          </Card>
        </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-1 gap-4 md:col-span-3">
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    ...
                    <p className="text-xl font-semibold">{formatTemperature(selectedDay.day.maxtemp_c, unit)}</p>
                  </div>
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <p className="text-sm text-gray-500">Min Temperature</p>
                    <p className="text-xl font-semibold">{formatTemperature(selectedDay.day.mintemp_c, unit)}</p>
                  </div>
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <p className="text-sm text-gray-500">Avg Temperature</p>
                    <p className="text-xl font-semibold">{formatTemperature(selectedDay.day.avgtemp_c, unit)}</p>
                  </div>
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <p className="text-sm text-gray-500">Max Wind</p>
                    <p className="text-xl font-semibold">
                      {unit === 'c' ? selectedDay.day.maxwind_kph : selectedDay.day.maxwind_mph} {unit === 'c' ? 'km/h' : 'mph'}
                    </p>
                  </div>
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <p className="text-sm text-gray-500">Total Precipitation</p>
                    <p className="text-xl font-semibold">
                      {formatPrecipitation(unit === 'c' ? selectedDay.day.totalprecip_mm : selectedDay.day.totalprecip_in, unit === 'c' ? 'mm' : 'in')}
                    </p>
                  </div>
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <p className="text-sm text-gray-500">Avg Humidity</p>
                    <p className="text-xl font-semibold">{selectedDay.day.avghumidity}%</p>
                  </div>
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <p className="text-sm text-gray-500">Chance of Rain</p>
                    <p className="text-xl font-semibold">{selectedDay.day.daily_chance_of_rain}%</p>
                  </div>
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <p className="text-sm text-gray-500">UV Index</p>
                    <p className="text-xl font-semibold">{selectedDay.day.uv}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 pt-6 border-t">
              ...
              <div className="overflow-x-auto pb-2">
                <div className="flex gap-4">
                  {selectedDay.hour.filter((_, i) => i % 3 === 0).map((hour) => (
                    <div
                      key={hour.time}
                    ...
                    >
                      ...
                      <div className="flex justify-between text-xs mt-2">
                        ...
                        <span>💨 {unit === 'c' ? hour.wind_kph : hour.wind_mph}</span>
                      </div>
                    </div>
                  ))}
                </div>
